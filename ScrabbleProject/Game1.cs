...
public class Game1 : Game
{
    ...
    protected override void Update(GameTime gameTime)
    {
        ...
        //call update for each GameObject
        for(int i = 0; i < gameObjects.Count; i++)
        {
    ...
    public void DrawRect(Vector2 pos, Vector2 size, bool centered, int thickness = 1, bool filled = true, Color color = default)
    {
        Vector2 centerPos = pos;
        if(!centered)
            centerPos = pos + new Vector2(size.X / 2, size.Y / 2);

        if(filled) //this works because thickness is added to both sides of the line
            DrawLine(centerPos, centerPos, (int)size.Y + thickness, color);
        else
        {
